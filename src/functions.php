...
namespace Bermuda\String {
    ...
    use function Bermuda\Array\to_array;
    ...
    function _string(string $text = '', ?string $encoding = null, bool $insensitive = false): _String
    {
        return new class($text, $encoding, $insensitive) implements _String {
             ...
             * @return _String
             */
            public function titleize(string|array $ignore = []): _String
            {
    ...
    {
        return str_starts_with(strrev($haystack), array_map('strrev', to_array($needle)), $insensitive);
    }
     ...
     * @return string|null
     */
    function str_before(string $subject, string $needle, bool $withNeedle = false):? string
    {
        $index = strpos($subject, $needle);
        if ($index === false) {
            return null;
        }
        return substr($subject, 0, $withNeedle ?  $index + mb_strlen($needle) : $index);
    }
     ...
     * @return string|null
     */
    function str_after(string $subject, string $needle, bool $withNeedle = false):? string
    {
        $index = strpos($subject, $needle);
        if ($index === false) {
            return null;
        ...
        return str_slice($subject, $withNeedle ? $index : $index + mb_strlen($needle));
    }
     ...
     * @return bool
     */
    function str_equals(string $haystack, string|array $needle, bool $insensitive = true): bool
    {
        !$insensitive ?: $haystack = strtolower($haystack);
        foreach (to_array($needle) as $value) {
            $result = $insensitive ? $haystack === strtolower((string) $value)
...
namespace Bermuda\Array {
    use Bermuda\Arrayable;
    if (!function_exists('\Bermuda\Array\to_array')) {
        function to_array(mixed $var): array
        {
            return is_array($var) ? $var : ($var instanceof Arrayable ? $var->toArray() : [$var]);
        }
    };
}
